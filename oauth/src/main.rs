...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    BasicClient::new(
        ...
        Some(TokenUrl::new(" https://sandbox-api.dexcom.com/v2/oauth2/token".to_string())?)
    )
    // Set the URL the user will be redirected to after the authorization process.
    .set_redirect_uri(RedirectUrl::new("http://172.28.244.153:3321/dexcom".to_string())?);
    ...
    for x in 1..100 {
        let body = reqwest::get(format!("http://172.28.244.153:3321/state?state={}",csrf_token.secret()))
        .await?
        .text()
        .await?;
